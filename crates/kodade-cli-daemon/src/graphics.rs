...
impl HandoffState {
    ...
    pub(crate) fn restore(self) -> (Store, Decoder, Tracker) {
        ...
        (self.store, self.decoder, tracker)
    }
}
...
mod tests {
    ...
    fn handoff_continues_partial_utf8_and_terminal_controls_at_every_byte() {
        ...
        let mut expected = vt100::Parser::new(12, 80, 100);
        expected.process(input);
        for split in 0..input.len() {
            let mut source = vt100::Parser::new(12, 80, 100);
            ...
            let state: HandoffState =
                serde_json::from_slice(&serde_json::to_vec(&state).unwrap()).unwrap();
            let mut restored = vt100::Parser::new(12, 80, 100);
            restored.process(&source.screen().state_formatted());
