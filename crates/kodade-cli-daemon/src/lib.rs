...
use kodade_cli_proto::{
    ...
    PaneSnapshot, QueryKind, Screen, ServerMessage, SplitAxis, TabId, TabInfo, WorkspaceId,
    WorkspaceInfo,
};
        ...
        match message {
            ...
            ClientMessage::FocusPaneId { id } => {
                ...
                for workspace in &mut state.workspaces {
                    for tab in &mut workspace.tabs {
                        if layout::contains(&tab.tree, id) {
                            tab.focused = id;
                            location = Some((workspace.id, tab.id));
                            break;
                        }
                    }
                    if location.is_some() {
                ...
                let workspace = state
                    ...
                    .expect("workspace containing pane exists");
