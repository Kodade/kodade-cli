...
impl Session {
        ...
        for saved in &file.workspaces {
            ...
            for saved_tab in &saved.tabs {
                ...
                for saved_pane in &saved_tab.panes {
                    ...
                    let new_id = session.new_pane_with_id(
                        session.pane_id(),
                        &saved_pane.title,
                        cwd,
                        command,
    ...
    fn new_pane(
        ...
        self.new_pane_with_id(self.pane_id(), title, cwd, command, HashMap::new())
    }
    ...
    fn new_pane_message(
        ...
        let pane = self.new_workspace_pane(target, &title, cwd, command)?;
    ...
    fn handle(&self, message: ClientMessage) -> Result<()> {
        match message {
            ...
            ClientMessage::SplitRight | ClientMessage::SplitDown => {
                ...
                let pane = self.new_pane_with_id(
                    self.pane_id(),
                    "shell",
                    cwd,
                    None,
