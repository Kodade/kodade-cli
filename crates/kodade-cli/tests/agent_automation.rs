...
fn hook_backed_node_agent_needs_no_osc_title() {
    ...
    harness.start(
        "hook-pi",
        "node -e 'const { spawnSync } = require(\"child_process\"); const report = state => spawnSync(process.env.KODADE_BIN, [\"-s\", process.env.KODADE_SESSION, \"agent\", \"report\", process.env.KODADE_PANE, state, \"--source\", \"kodade:pi\", \"--native-agent\", \"pi\"]); report(\"working\"); process.stdin.on(\"data\", () => report(\"done\")); setTimeout(() => {}, 10000);'",
    );
