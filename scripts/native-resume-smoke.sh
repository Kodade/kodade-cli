...
try:
    ...
    for pane, session in zip(panes,("first","second")):
        run("-s","native-smoke","agent","report",pane,"working","--source","kodade:codex","--native-agent","codex","--hook-json",input=json.dumps({"session_id":session,"nested":{"session_id":"decoy"}}))
