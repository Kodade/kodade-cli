...
# Wait for this pane's hook to finish before comparing post-disable log size.
# An earlier in-flight hook may legitimately append after disable returns.
pane=$("$bin" --session plugins ls --json | python3 -c 'import json,sys; print(next(p["id"] for p in json.load(sys.stdin)["panes"] if p["focused"]))')
for _ in $(seq 1 80); do
  grep -q "opened $pane" "$HOME/.config/kodade-cli/plugins/logs/hello-plugin.log" 2>/dev/null && break
  sleep 0.1
done
grep -q "opened $pane" "$HOME/.config/kodade-cli/plugins/logs/hello-plugin.log"
